...
namespace ServerNetworkModule
{
	...
	class CMessageResolver
	{
		...
		byte[] message_buffer = new byte[1024];	    // 진행중인 버퍼.
		int current_position;						// 현재 진행중인 버퍼의 인덱스를 가리키는 변수. 패킷 하나를 완성한 뒤에는 0으로 초기화 시켜줘야 한다.
		...
		bool read_until(byte[] buffer, ref int src_position, int offset, int transffered)
		{
			...
			if (this.remain_bytes < copy_size) copy_size = this.remain_bytes;
			
			// 버퍼에 복사.
			Array.Copy(buffer, src_position, this.message_buffer, this.current_position, copy_size);
		
			src_position += copy_size;				// 원본 버퍼 포지션 이동.
			this.current_position += copy_size;		// 타겟 버퍼 포지션도 이동.
			this.remain_bytes -= copy_size;			// 남은 바이트 수.
		...
		/// <summary>
		/// Socket Buffer로부터 데이터 수신이 발생할 때마다 호출
		...
		{
			int src_position = offset;			// 원본 버퍼의 포지션값. 패킷이 여러개 뭉쳐 올 경우 원본 버퍼의 포지션은 계속 앞으로 가야 함
			this.remain_bytes = transffered;	// 이번 receive로 읽어야할 바이트 수.

			while (this.remain_bytes > 0)	// 남은 데이터가 있다면 계속 반복한다.
			{ 
				...
				{				
					this.position_to_read = Defines.HEADERSIZE;     // 목표 지점 설정(헤더 위치까지 도달하도록 설정).

					completed = read_until(buffer, ref src_position, offset, transffered);
					if (!completed) return; // 아직 다 못읽었으면 다음 receive를 기다린다.
							
					this.message_size = get_body_size();								// 헤더 하나를 온전히 읽어왔으므로 메시지 사이즈를 구한다.
					this.position_to_read = this.message_size + Defines.HEADERSIZE;		// 다음 목표 지점(헤더 + 메시지 사이즈).
				}
			
				completed = read_until(buffer, ref src_position, offset, transffered);   // 메시지를 읽는다.
		...
		int get_body_size()
		{
			...
			return type.Equals(typeof(Int16)) ? BitConverter.ToInt16(this.message_buffer, 0) : BitConverter.ToInt32(this.message_buffer, 0);
		}
